...
def get_latest_trained_model_path():
    ...
    """
    latest_run_dir = os.listdir("./results")
    latest_run_dir.sort()
    latest_run_dir = latest_run_dir[-1]
    saved_models_path = "./results/" + latest_run_dir + "/Saved_Models"
    generated_images_path = "./results/" + latest_run_dir + "/Generated_Images"
    tensorboard_path = "./results/" + latest_run_dir + "/Tensorboard"
...
def train(z_dim, batch_size, learning_rate, beta1, n_iter, image_size, load=False):
    ...
    real_img = load_dataset(mc.DATASET_PATH, mc.LABEL_PATH, image_size=image_size)
    ...
    labels_values = tf.placeholder(tf.float32, [batch_size] + [1], name="real_labels")
    ...
    all_ones = tf.ones_like(Dx)

    # Loss
    d_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=Dx, labels=labels_values))
    d_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=Dg, labels=tf.zeros_like(Dg)))
    gloss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=Dg, labels=labels_values))
    ...
    with tf.Session() as sess:
        ...
        if not load:
            for idx in range(n_iter):
                batch_data = next_batch(real_img, batch_size=batch_size, image_size=image_size)
                batch_images = batch_data.data_sets
                batch_labels = batch_data.labels
                batch_labels = np.reshape(batch_labels, (12, 1)).astype(np.float32)
                batch_z = np.random.uniform(-1, 1, [batch_size, z_dim]).astype(np.float32)

                for k in range(1):
                    sess.run([d_optim], feed_dict={images: batch_images, labels_values: batch_labels, zin: batch_z})
                for k in range(1):
                    sess.run([g_optim], feed_dict={zin: batch_z, labels_values: batch_labels})

                print("[%4d/%4d] time: %4.4f, " % (idx, n_iter, time.time() - start_time))

                if idx % 10 == 0:
                    # Display the loss and run tf summaries
                    summary = sess.run(summary_op, feed_dict={images: batch_images, labels_values: batch_labels, zin: batch_z})
                    writer.add_summary(summary, global_step=idx)
                    d_loss = d_loss_fake.eval({zin: display_z, images: batch_images, labels_values: batch_labels})
                    g_loss = gloss.eval({zin: batch_z, labels_values: batch_labels})
